            ...
            for line in doc.page_content.splitlines():
                if line.startswith("Professor: "):
                    ...
                    if name not in seen:
                        ...
                        seen.add(name)
        if profs:
            names = "\n".join([f"{i+1}. {name}" for i, name in enumerate(profs)])
            return {"answer": f"Professors who teach {course_code}:\n{names}"}

    # === Direct answer if professor matched
    if professor_name:
        courses = []
        seen = set()
        for doc in matching_docs:
            for line in doc.page_content.splitlines():
                if line.startswith("Course Code: "):
                    ...
                    if code not in seen:
                        ...
                        seen.add(code)
        if courses:
            names = "\n".join([f"{i+1}. {name}" for i, name in enumerate(courses)])
            return {"answer": f"Courses taught by {professor_name}:\n{names}"}
